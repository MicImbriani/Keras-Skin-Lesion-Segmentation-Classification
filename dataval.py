...
def to_npy(path, is_augmented):
    ...
    if is_augmented:
        data_file = glob.glob(path + '/ISIC-2017_Validation_Data/*.png')
    else:
        data_file = glob.glob(path + '/ISIC-2017_Validation_Data/*.jpg')
    files = []

    data_file_mask = glob.glob(path + '/ISIC-2017_Validation_Part1_GroundTruth/*.png')
    ...
    for f in (data_file):
        ...
        base = os.path.basename(path + "/ISIC-2017_Validation_Data/" + f)
        fileName = os.path.splitext(base)[0]
        files.append(fileName)
        i += 1
        
    for k in (data_file_mask):
        base = os.path.basename(path + "/ISIC-2017_Validation_Part1_GroundTruth/" + k)
        fileName = os.path.splitext(base)[0]
