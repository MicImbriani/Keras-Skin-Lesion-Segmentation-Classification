...
def conv_block(neurons, block_input, batch_norm=False, dropout=None, middle=False):
    conv1 = Conv2D(neurons, (3,3), activation='relu', padding='valid', kernel_initializer='he_normal')(block_input)
    if batch_norm:
        conv1 = BatchNormalization()(conv1)
    if dropout is not None:
        conv1 = SpatialDropout2D(dropout)(conv1)
    conv2 = Conv2D(neurons, (3,3), activation='relu', padding='valid', kernel_initializer='he_normal')(conv1)
    if batch_norm:
...
def deconv_block(neurons, block_input, shortcut, batch_norm=False, dropout=None):
    deconv = Conv2DTranspose(neurons, (2, 2), activation='relu', strides=(2, 2), padding="valid")(block_input)
    uconv = concatenate([deconv, shortcut])
    uconv = Conv2D(neurons, (2, 2), padding="valid", kernel_initializer='he_normal')(uconv)
    if batch_norm:
        uconv = BatchNormalization()(uconv)
    #uconv = Activation('relu')(uconv)
    if dropout is not None:
        uconv = SpatialDropout2D(dropout)(uconv)
    uconv = Conv2D(neurons, (2, 2), activation='relu', padding="valid", kernel_initializer='he_normal')(uconv)
    if batch_norm:
        uconv = BatchNormalization()(uconv)
    #uconv = Activation('relu')(uconv)
    if dropout is not None:
...
def build_model(input_size, batch_norm=False):
    ...
    #final_conv = Conv2D(2, (3,3), activation='relu', padding='valid', kernel_initializer='he_normal')(deconv1)

    output_layer = Conv2D(1, (1,1), padding="valid", activation="sigmoid")(deconv1)
